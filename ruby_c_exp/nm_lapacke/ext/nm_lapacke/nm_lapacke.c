...
static VALUE call_gesv(VALUE coeffs, VALUE rhs)
{
...
void Init_nm_lapacke()
{
  VALUE cNMatrix = rb_define_class("NMatrix",rb_cObject);
  VALUE mLAPACKE = rb_define_module_under(cNMatrix, "LAPACKE");
  rb_define_singleton_method(mLAPACKE, "__solve_gesv__", solve_gesv, 2);
}